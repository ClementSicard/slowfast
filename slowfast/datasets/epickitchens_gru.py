...
from .epickitchens_record_gru import EpicKitchensVideoRecordGRU
...
class EpicKitchensGRU(torch.utils.data.Dataset):
    def __init__(self, cfg, mode):
        ...
        logger.info("Constructing {} {}...".format(self.__class__.__name__, mode))
    ...
    def __getitem__(self, index):
        ...
        video_record = self._video_records[index]
        ...
        for _ in range(min(num_subclips, self.cfg.DATA.MAX_NB_SUBCLIPS)):
            ...
            fast_frames.append(fast_frame)

        slow_frames = torch.stack(slow_frames, dim=0)
        fast_frames = torch.stack(fast_frames, dim=0)
